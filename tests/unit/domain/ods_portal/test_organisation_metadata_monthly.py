...
def test_from_list_of_data_returns_dict_of_organisation_metadata():
    ...
    expected_first_month_icbs = [IcbMetadata(ods_code="12A", name="ICB", practices=["A12345"])]
    expected_second_month_icbs = [IcbMetadata(ods_code="22A", name="ICB", practices=["A12345"])]
    expected_first_month_lookup = OrganisationLookup(
        expected_first_month_practices, expected_first_month_icbs, year_month=(2020, 6)
    )
    expected_second_month_lookup = OrganisationLookup(
        expected_second_month_practices, expected_second_month_icbs, year_month=(2020, 7)
    )
